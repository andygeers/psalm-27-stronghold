...
* We'll also start the player off there with 10 points: ``||info:set score 10||``
* Then ``||game:on game update every 1000ms||`` let's ``||info:set score||`` to ``||info:countdown||``. So if you complete it with 6 seconds left on the clock, you get 6 points, if you complete it with 4 seconds left on the clock, you get 4 points, and so on.
